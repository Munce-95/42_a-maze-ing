...
def is_obstacle(cell: Cell) -> bool:
    ...
    -------
    bool
        True if the cell has to be considered as an obstacle, False otherwise.
    """
...
def non_perfect(
        ...
        parsed: Data) -> None:
    """
    Remove dead-ends from the maze by repeatedly breaking walls.
...
def apply_pattern(
    ...
    """
    Pregenerate the pattern within the maze's edge.
    ...
    -------
    Tuple[bool, List[str]]
        A tuple containing a boolean indicating if the sans pattern is used
        and the list of colors from the themes file.
    """
...
def get_unblocked_neighbors(
    ...
    """
    Get the list of available cells to move into.
    ...
    grid : List[List[Cell]]
        The maze generated by Wilson's algorithm.
    cell : Cell
        The cell whose neighbors are being searched.
    parsed : Data
        All the parsed data from the config.txt file.
    ...
    -------
    List[Cell]
        The list of cells that are not obstacles.
    """
    neighbors = []
...
def generate_wilson(parsed: Data) -> Tuple[List[List[Cell]], bool, list[str]]:
    """
    Generate the whole maze using Wilson's Algorithm.
...
def build_matrix_1x1(
    ...
    """
    Build the maze matrix with characters.
    ...
    parsed : Data
        All the parsed data from the config.txt file.
    is_sans : bool, optional
        A boolean to check if the sans pattern is used, by default False.

    Returns
    -------
    Tuple[List[List[str]], int, int]
        A tuple containing the maze's character matrix used in the render,
...
def _merge_sans_blocks(
        ...
        parsed: Data,) -> None:
    """
    Merge the walls inside of the sans pattern.
    ...
    grid : List[List[Cell]]
        The maze generated by Wilson's algorithm.
    maze_matrix : List[List[str]]
        The matrix representation of the maze.
    parsed : Data
        All the parsed data from the config.txt file.
    """
    for x in range(parsed.width):
...
def render_terminal_blocks(
    ...
    """
    Render the maze using ANSI escape codes with the selected color palette.

    Parameters
    ----------
    matrix : List[List[str]]
        The character matrix representation of the maze.
    render_w : int
        The width of the rendered character grid.
    render_h : int
        The height of the rendered character grid.
    bg_list : List[str]
        The list of color codes from the themes file.
    """
...
def handle_sigquit(sign: Any, frame: Any) -> None:
    """
    Handle the SIGQUIT signal to exit the program cleanly.
...
def wilson_main(parsed: Data) -> Tuple[List[List[str]],
    ...
    Returns
    -------
    Tuple[List[List[str]], int, int, bool, List[str]]
        A tuple containing the maze matrix, render width, render height,
        is_sans flag, and color palette list.
    """
